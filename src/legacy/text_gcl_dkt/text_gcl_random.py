...
hyper_params = {
    ...
    "model_params": {
        ...
        "g_layers": 2,
        "g_dropout": 0.1,
        "text_emb_model": "/home/duanzt/model_weights/Qwen-Embedding/Qwen3-Embedding-0.6B",
    ...
    "exp_params": {
        ...
        "num_workers": 0,
        "pin_memory": False,
        "gcl_lambda": 0.2,      # 辅助任务的权重 lambda
        "gcl_temp": 0.2,        # InfoNCE temperature
        "gcl_drop_edge": 0.3,   # 随机删边概率
        "gcl_mask_feat": 0.3    # 随机特征掩码概率
    }
}
...
class PathConfig:
    def __init__(self):
        ...
        self.data_dir = self.base_dir / "data" / "ktdata" / "mooc_processed"
        self.folds_dir = self.data_dir # /home/duanzt/data/ktdata/mooc_processed
        self.cache_dir = self.base_dir / "cache"
        self.cache_dir.mkdir(exist_ok=True)
        # <--- 修改：修正 embed_model_dir 的路径逻辑
        self.embed_model_dir = Path(hyper_params["model_params"]["text_emb_model"])
        # 如果模型路径是相对路径，则基于 base_dir
...
class EmbeddingCache:
    ...
    def _compute_hash(self, texts: list, model_dir: Path, max_length: int) -> str:
        # 使用 model_dir 的名称而不是完整路径来哈希，更稳定
    ...
    def get_embeddings(self, texts: list, model_dir: Path, device=None, 
        ...
        # 改用 .pt 后缀，使用 torch 原生方法保存，避免 pickle 问题
        cache_file = self.cache_dir / f"embeddings_{cache_hash}.pt"
        
        # 自动清理可能导致报错的旧 .pkl 文件
        old_broken_file = self.cache_dir / f"embeddings_{cache_hash}.pkl"
        if old_broken_file.exists():
            try:
                os.remove(old_broken_file)
                print(f"[Cache] 已删除旧的损坏缓存文件: {old_broken_file}")
            except OSError:
                pass
        ...
        if cache_file.exists() and not force_recompute:
            ...
            try:
                # 使用 torch.load 代替 pickle
                # map_location="cpu" 确保加载时不占用显存，后续再 .to(device)
                t = torch.load(cache_file, map_location="cpu")
                # 确保加载出来的是 float32
                t = t.to(dtype=torch.float32)
                return t.to(device) if device is not None else t
            except Exception as e:
                print(f"[Cache] 读取失败 ({e})，文件可能已损坏，删除并重新计算...")
                try:
        ...
        # 关键修改：统一转换为 float32，避免 BFloat16 vs Float 冲突
        embs = embs.to(dtype=torch.float32)
        
        torch.save(embs.cpu(), cache_file)
...
def build_graph_with_text(eid2sids, eid2desc, sid2desc, path_config, model_params, device):
    ...
    for eid in exercise_ids:
        desc_data = eid2desc.get(str(eid), {})
        if isinstance(desc_data, dict):
            ex_texts_mem.append(desc_data.get("memory", ""))
            ex_texts_und.append(desc_data.get("understand", ""))
            ex_texts_skl.append(desc_data.get("skill", ""))
        else:
            # 兼容旧格式（纯文本）
            ex_texts_mem.append(str(desc_data))
            ex_texts_und.append("")
            ex_texts_skl.append("")
    ...
    # <--- 修改：分别计算三类文本的 embedding
    print("[图构建] 计算 Exercise Memory Embedding...")
    ...
    print("[图构建] 计算 Exercise Understand Embedding...")
    ex_emb_und = cache.get_embeddings(ex_texts_und, text_emb_model_dir, device=device,
                                  batch_size=model_params['text_emb_batch_size'],
                                  max_length=model_params['text_emb_max_len'])
    
    print("[图构建] 计算 Exercise Skill Embedding...")
    ...
    # <--- 修改：融合机制 (Concatenation + Linear Projection)
    # 将三个 embedding 拼接：[N, H] * 3 -> [N, 3H]
    ex_emb_cat = torch.cat([ex_emb_mem, ex_emb_und, ex_emb_skl], dim=1)
    
    if ex_emb_cat.numel() > 0:
        # 确保输入是 float32，防止 Linear 层类型不匹配
        ex_emb_cat = ex_emb_cat.to(dtype=torch.float32)
        
        # 无论维度是否匹配，只要不是 target_dim，都进行投影
        # 注意：这里使用随机初始化的 Linear 层进行降维融合
        if ex_emb_cat.shape[1] != target_dim:
            print(f"[图构建] 融合 Exercise Embedding: {ex_emb_cat.shape[1]} -> {target_dim}")
            proj = Linear(ex_emb_cat.shape[1], target_dim).to(device)
            with torch.no_grad(): 
                ex_emb = proj(ex_emb_cat)
        else:
            ex_emb = ex_emb_cat
    ...
    if sk_emb.numel() > 0:
        # 确保输入是 float32
        sk_emb = sk_emb.to(dtype=torch.float32)
...
class Experiment:
    ...
    def train_one_epoch(self, train_loader: DataLoader) -> Dict[str, float]:
        ...
        # 超参数：数据集超参数（手动），模型超参数（不变），训练超参数（自动调整）
        metrics = {
...
def get_global_info(path_config: PathConfig, n_folds: int):
    ...
    # <--- 修改：加载新的 jsonl 格式 eid2desc
    eid2desc = {}
    jsonl_path = path_config.folds_dir / "eid2desc_rewrite2.jsonl"
    if jsonl_path.exists():
        print(f"加载题目文本: {jsonl_path}")
        with open(jsonl_path, "r", encoding="utf-8") as f:
            for line in f:
                ...
                try:
                    obj = json.loads(line)
                    # obj 格式: {"eid": "...", "desc": {"memory": "...", ...}}
                    eid2desc[str(obj["eid"])] = obj["desc"]
                except json.JSONDecodeError:
                    continue
    else:
        print(f"警告: 未找到 {jsonl_path}，尝试加载旧版 json")
