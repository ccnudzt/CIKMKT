...
hyper_params = {
    ...
    "exp_params": {
        ...
        "batch_size": 512,
        "lr": 0.01,
        "num_epochs": 100, # 定向扰动概率大，需要增加更多的训练轮数
        "weight_decay": 1e-5,
        "num_workers": 0,
        "pin_memory": False,
        
        # <--- 修改：定向扰动对比学习参数
        "gcl_lambda": 0.4,       # Loss权重
        "gcl_temp": 0.2,         # Temperature
        "gcl_p_min": 0.05,       # 最小丢弃概率 (保护长尾/冷门)
        "gcl_p_max": 0.3,        # 最大丢弃概率 (扰动热门/冗余)
        "gcl_mask_feat": 0.1     # 特征掩码依然保留随机
    }
}
...
class GraphCLUtil(Module):
    def __init__(self, temperature=0.2, mask_feat_rate=0.1):
        ...
        # 注意：drop_edge_rate 不再是固定值，而是通过 external probability 传入
        self.mask_feat_rate = mask_feat_rate

    def augment(self, data: HeteroData) -> HeteroData:
        """生成图的一个视图：基于概率的定向删边 + 随机特征掩码"""
        view = data.clone()
        
        # --- 1. 定向删边 (Structure-aware Edge Dropping) ---
        ...
        if edge_type in view.edge_index_dict:
            ...
            if hasattr(view[edge_type], 'edge_drop_prob'):
                ...
                # 伯努利采样: torch.bernoulli(p) 返回 1 的概率是 p
                # 我们需要保留的 mask (keep_mask)，如果 drop_prob 是丢弃概率
                # 那么 keep_prob = 1 - drop_prob
                keep_probs = 1.0 - probs
                keep_mask = torch.bernoulli(keep_probs).to(torch.bool)
                
                view[edge_type].edge_index = edge_index[:, keep_mask]
            else:
                # 如果没有预计算（兼容性兜底），使用默认随机 10%
    ...
    def compute_infonce(self, z1: torch.Tensor, z2: torch.Tensor) -> torch.Tensor:
        ...
        # 相似度矩阵: [N, N]
        # sim_matrix[i, j] = cos(z1_i, z2_j) / temp
        sim_matrix = torch.mm(z1, z2.t()) / self.temperature
        
        # 标签: 对角线是正样本 (i, i)
        labels = torch.arange(z1.size(0), device=z1.device)
        
        # CrossEntropy 自动对每一行做 Softmax
        # 分子: exp(sim(i, i)/t)
        # 分母: sum_k exp(sim(i, k)/t)  <-- 这里包含了同类别的所有其他节点作为负样本
...
def precompute_edge_drop_probs(data: HeteroData, p_min: float, p_max: float):
    """
    根据节点度数计算每条边的丢弃概率，实现了基于结构重要性的定向扰动。
    公式: w_ij = log(d_q) + log(d_k)
          p_ij = min(p_max, p_min + (p_max - p_min) * norm(w_ij))
    """
    print("[GCL] 正在预计算基于结构重要性的边丢弃概率...")
    ...
    # 4. 映射到 [p_min, p_max]
    # 权重越大(度数越大)，越容易被丢弃(p 接近 p_max)
    drop_probs = p_min + (p_max - p_min) * w_norm
    drop_probs = torch.clamp(drop_probs, max=p_max)
    
    # 5. 将概率存储在 data 对象中，以便后续使用
    data[edge_type].edge_drop_prob = drop_probs
    
    print(f"[GCL] 预计算完成. Prob Range: [{drop_probs.min():.4f}, {drop_probs.max():.4f}], Mean: {drop_probs.mean():.4f}")
    return data
