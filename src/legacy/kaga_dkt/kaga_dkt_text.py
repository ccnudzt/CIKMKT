...
class KAGADKT(Module):
    def __init__(self, model_params: Dict[str, Any]):
        ...
        self.ans_embed = Embedding(3, self.embed_dim, padding_idx=0) # 0:pad, 1:wrong, 2:right
    ...
    # <--- 修改：增加 mask 参数
    def forward(self, eids: torch.Tensor, is_corrects: torch.Tensor, mask: torch.Tensor):
        assert self._graph_data is not None and self._eid_lookup is not None, "DKT 未设置图数据"
        
        # 1. GNN 编码只在需要时运行一次（如果图是静态的）
        # (在训练循环中，self.gnn(data) 会被多次调用，但如果 data 不变，结果不变)
        # 为确保特征是最新的，每次都调用
        self.gnn(self._graph_data)
        ex_seq = self.gnn.gather_exercise_seq(eids, self._eid_lookup)
        
        # <--- 修改：修复答案嵌入的逻辑
        # is_corrects: 0 (wrong), 1 (right)
        # 我们想要: 1 (wrong), 2 (right)
        ans_idx = is_corrects + 1 
        # 使用 mask 将 padding 的位置 (原先是 0) 设置为 0 (padding_idx)
...
class Experiment:
    ...
    def reset_config(self):
        ...
        self.model = KAGADKT(self.model_params).to(self.device)
    ...
    def train_one_epoch(self, train_loader: DataLoader) -> Dict[str, float]:
        ...
        pbar = tqdm(train_loader, desc=f"Training Epoch {self.current_epoch:02d}", leave=False)
        ...
        if y_true.size == 0:
            return {"train_loss": float("nan"), "train_auc": float("nan"), "train_acc": float("nan")}

        auc = roc_auc_score(y_true, y_prob) if len(np.unique(y_true)) > 1 else 0.5 # <--- 修改：处理单类别情况
        acc = accuracy_score(y_true, (y_prob >= 0.5).astype(int))
        return {"train_loss": np.mean(epoch_losses), "train_auc": auc, "train_acc": acc}
