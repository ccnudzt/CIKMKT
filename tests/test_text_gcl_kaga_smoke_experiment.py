...
def test_text_gcl_dkt_smoke_experiment(tmp_path: Path):
    recordings_path = tmp_path / "recordings.jsonl"
    eid2sid_path = tmp_path / "eid2sid.json"
    eid2desc_path = tmp_path / "eid2desc.json"
    sid2desc_path = tmp_path / "sid2desc.json"
    ...
    eid2desc_path.write_text(
        json.dumps(
            {
                "1": {"memory": "m1", "understand": "u1", "skill": "s1"},
                "2": {"memory": "m2", "understand": "u2", "skill": "s2"},
                "3": {"memory": "m3", "understand": "u3", "skill": "s3"},
                "4": {"memory": "m4", "understand": "u4", "skill": "s4"},
            },
            ensure_ascii=False,
        ),
        encoding="utf-8",
    )
    sid2desc_path.write_text(json.dumps({"11": "k1", "12": "k2", "13": "k3"}, ensure_ascii=False), encoding="utf-8")
    ...
    config = {
        "runtime": {
            ...
            "experiment_name": "text_gcl_dkt_smoke",
            "save_last": True,
            "num_workers": 0,
        },
        "data": {
            "builder": "text_graph_kt",
            "dataset_name": "toy_text_graph",
            "recordings_path": str(recordings_path),
            "eid2sid_path": str(eid2sid_path),
            "eid2desc_path": str(eid2desc_path),
            "sid2desc_path": str(sid2desc_path),
            "min_len": 2,
            "max_len": 8,
            "num_folds": 2,
            "r_pad": 0,
    ...
    config_path = tmp_path / "text_gcl.yaml"
    config_path.write_text(yaml.safe_dump(config, sort_keys=False), encoding="utf-8")
    summary = run_experiment(config_path)
    assert summary["num_folds"] == 2
    assert (tmp_path / "outputs" / "text_gcl_dkt_smoke" / "metrics.json").exists()
